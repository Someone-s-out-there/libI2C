...
static inline ret_code_t twi_enable_isr() {
    TWCR |= (1 << TWIE);
    return SUCCESS;
}
...
#define TWISENDACK()                                   \
    (TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWIE) | \
            (1 << TWEA))  // FOR MR mode. Resume a transfer, ensure that TWI and
                          // interrupts are enabled and respond with an ACK if
                          // the device is addressed as a slave or after it
                          // receives a byte.

#define TWISENDNACK()                    \
    (TWCR = (1 << TWINT) | (1 << TWEN) | \
            (1 << TWIE))  // FOR MR mode. Resume a transfer, ensure that TWI and
                          // interrupts are enabled but DO NOT respond with an
                          // ACK if the device is addressed as a slave or after
                          // it receives a byte.

#define TWI_MRX_SLAR_ACK 0x40
#define TWI_MRX_DATA_ACK 0x50
#define TWI_MRX_DATA_NACK 0x58
#define RX_BUFF_L 20

uint8_t RX_buffer[RX_BUFF_L];
uint8_t RX_buffer_idx = 0;

ISR(TWI_vect) {
    switch (TWI_STATUS) {
        case TWI_MRX_SLAR_ACK:
            if (RX_buffer_idx < RX_BUFF_L - 1) {
                TWISENDACK();
            } else {
                TWISENDNACK();
            }
